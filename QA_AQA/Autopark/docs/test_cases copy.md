...
- User: register phone too short — Ожид.: 422/400 — Pass/Fail (зависит от валидации) — `...::TestUserValidationMore::test_register_phone_too_short`
- User: register minimal invalid email — Ожид.: 422/400 — Pass/Fail (зависит от валидации) — `...::TestUserValidationMore::test_register_email_minimal_invalid`
- User: register phone with special symbols — Ожид.: 422/400 — Fail (получен 201) — `...::TestUserValidationMore::test_register_phone_with_special_symbols` — См. `#BUG-USER-PHONE-SYMBOLS`
...
_*Fail/Fail* - зафиксирована текущая серверная логика: invalid email может приводить к 201 (создание) или 409 (конфликт), ожидается 422/400. Проблема задокументирована в intern.md._
...
| TC-AUTH-006  | Refresh token: валидный                         | получить refresh_token        | 1. POST /auth/refresh-access-token с токеном                                                    | 200/201, новый access_token         | Pass   | Smoke     | test_auth.py     |
...
| TC-AUTH-009  | Change-password: позитивный сценарий            | валидный USER/PASS            | 1) POST /auth/change-password (старый->новый) 2) Login новым 3) POST /auth/change-password (обратно) | 200/201/202                         | Pass   | Smoke     | test_auth.py     |
...
| TC-USER-013  | Регистрация: телефон со спецсимволами           | уникальный email             | 1. POST /user с phone="+7(999)-12*3-45#67"                                                       | 422/400                             | Fail   | Regression| test_user.py     |
...
| TC-VEH-010   | POST /vehicle: полный payload                     | валидный токен, type_id      | 1. POST /vehicle со всеми полями (brand, type_id, state_number, name, load_capacity, max_cargo_*) | 201, все поля сохранены            | Pass   | Regression| test_vehicle.py  |
| TC-VEH-011   | PATCH /vehicle: полный payload                   | валидный токен, валидный id  | 1. PATCH /vehicle со всеми обновляемыми полями (brand, name, load_capacity, max_cargo_*)        | 200/202, все поля обновлены        | Pass   | Regression| test_vehicle.py  |

_*Fail/Fail* - зафиксирована текущая серверная логика: invalid email даёт 409; сообщено в intern.md._

Примечание:
- Для PATCH Vehicle используется минимально совместимый набор полей по OpenAPI: только `id` обязателен, обновляем безопасно `brand` и `name`. Это устранило 422.
- Добавлены позитивные тесты с максимально полным payload (`TC-VEH-010`, `TC-VEH-011`) — проверяют создание/обновление со всеми доступными полями.
- Все Vehicle тесты используют динамическое получение `type_id` через `GET /vehicle-type/all` вместо захардкоженного значения.
...
Список дополняется по мере расширения тестов. Все подробности по статусу в `/docs/intern.md` и `reports/report.html`.
