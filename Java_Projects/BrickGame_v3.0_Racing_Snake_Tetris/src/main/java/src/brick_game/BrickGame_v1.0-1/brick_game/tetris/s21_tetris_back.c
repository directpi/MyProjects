...
action fsm_table[8][8] = {
    ...
    {spawn, NULL, game_exit, NULL, NULL, NULL, NULL, NULL},
    {NULL, NULL, game_exit, NULL, NULL, NULL, NULL, NULL},
...
void check_high_score(GameInfo_t *game, GameState_t *g_state) {
  int high_score = read_file_high_score();
  if (high_score != -1) {
    if (game->score > high_score) {
      write_to_file_high_score(game->score);
...
int read_file_high_score(void) {
  FILE *myfile = NULL;
  myfile = fopen("brick_game/tetris/record_score.txt", "r");
  if (!myfile) {
    write_to_file_high_score(0);
    myfile = fopen("brick_game/tetris/record_score.txt", "r");
  }
  int high_score = 0;
  if (myfile) {
    fscanf(myfile, "%d", &high_score);
    fclose(myfile);
  } else {
    printf("file brick_game/tetris/record_score.txt does not exist");
    high_score = -1;
  }
  return high_score;
}
...
int write_to_file_high_score(int score) {
  FILE *myfile = NULL;
  int error = 0;
  myfile = fopen("brick_game/tetris/record_score.txt", "w");
  if (myfile) {
...
GameInfo_t *updateCurrentState(UserAction_t u_action) {
  ...
  // Снимаем паузу ТОЛЬКО при повторном нажатии Pause
  if (g_state == PAUSE_STATE && u_action == Pause) {
    game.pause = 0;
    g_state = MOVING_STATE;
    // Возвращаем игру сразу, не вызывая fsm_table снова
    return &game;
  }
  clear_field(game.next);
  user_action = u_action;
  action act = fsm_table[g_state][u_action];
  if (act) act();
  
  // FIX: Блокируем переход в EXIT_STATE, если действие не было Terminate
  if (g_state == EXIT_STATE && u_action != Terminate) {
    g_state = MOVING_STATE;  // Возвращаем в нормальное состояние
    game.pause = 0;          // Сбрасываем pause, установленный game_exit()
  }
